//Best approach for putting  APIs together:
// sceed_frontend/src/config/api.js

const API_BASE_URL = "https://sceedbackend.pensoft.co.ke/api";
...
export const API_ENDPOINTS = {
  ...
  // Stock APIs
  getStock: (id) => `${API_BASE_URL}/products/${id}/stock`, // Retrieve stock for a product by ID
  ...
  // Order APIs
  createOrder: `${API_BASE_URL}/orders`, // Create a new order
  getOrder: (id) => `${API_BASE_URL}/orders/${id}`, // Retrieve an order by ID
  getUserOrders: (userId) => `${API_BASE_URL}/orders/user/${userId}`, // Retrieve user orders by user ID

  // Shipping APIs
  calculateShipping: (method) =>
    `${API_BASE_URL}/orders/shipping-cost/${method}`, // Calculate shipping cost by method

  // Payment APIs
  processPayment: `${API_BASE_URL}/payments/process`, // Process payment
  verifyPayment: (paymentId) => `${API_BASE_URL}/payments/verify/${paymentId}`, // Verify payment by payment ID
